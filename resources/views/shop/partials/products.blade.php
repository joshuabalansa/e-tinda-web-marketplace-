      ...
      <a href="{{ route('shop.product.show', $product->id) }}" class="product-link">
        <div class="card h-100 product-card">
          ...
          <div class="card-body d-flex flex-column">
            ...
            @if(isset($product->cooperative) && $product->cooperative)
              <p class="card-text mb-2">
                <a href="{{ route('cooperatives.show', $product->cooperative['slug'] ?? $product->cooperative['id']) }}" class="text-decoration-none" onclick="event.stopPropagation();">
                  <span class="badge bg-info">
                    <i class="fas fa-store"></i> {{ $product->cooperative['name'] }}
                  </span>
                </a>
              </p>
            @endif
